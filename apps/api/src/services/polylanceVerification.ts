...
export async function verifyPolyLanceCredential(inputString: string): Promise<VerificationResult> {
  ...
  try {
    ...
    const sbtRecords: any[] = await prisma.$queryRaw`
      SELECT * FROM "CertifiedSBTRecord"
      WHERE LOWER("id") = LOWER(${key})
         OR LOWER("jobId") = LOWER(${key})
         OR LOWER("sbtTokenId") = LOWER(${key})
         OR LOWER("contractAddress") = LOWER(${key})
         OR LOWER("ipfsCid") = LOWER(${key})
         OR LOWER("oracleSignature") = LOWER(${key})
         OR LOWER("freelancerAddress") = LOWER(${key})
         OR LOWER("clientAddress") = LOWER(${key})
         OR "id" ILIKE ${likePattern}
         OR "jobId" ILIKE ${likePattern}
         OR "contractAddress" ILIKE ${likePattern}
         OR "sbtTokenId" ILIKE ${likePattern}
         OR "ipfsCid" ILIKE ${likePattern}
         OR "freelancerAddress" ILIKE ${likePattern}
         OR "clientAddress" ILIKE ${likePattern}
      LIMIT 1;`;
